...
public class MapLoader {
    ...
    public MapLoader (String path) {
        ...
        try {
            vertexFile = Files.readString(Paths.get(path + "/collision_vertices.txt")).replace("\r", "").replace("\n", "");
            polygonFile = Files.readString(Paths.get(path + "/collision_polygons.txt")).replace("\r", "").replace("\n", "");
            collisionInfoFile = Files.readString(Paths.get(path + "/collision_info.txt")).strip().replace("\r", "").replace("\n\n", "\n");

            teamPlayerSpawnFile = Files.readString(Paths.get(path + "/spawns/t.txt")).replace("\r", "").replace("\n", ", ");
            ffaPlayerSpawnFile = Files.readString(Paths.get(path + "/spawns/b.txt")).replace("\r", "").replace("\n", ", ");
            teamItemSpawnFile = Files.readString(Paths.get(path + "/pickups/t.txt")).replace("\r", "").replace("\n", ", ");
            ffaItemSpawnFile = Files.readString(Paths.get(path + "/pickups/b.txt")).replace("\r", "").replace("\n", ", ");
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        fatherRotation = ExoRotation.getRotation(getProperty(collisionInfoStrings, "father_rotation"));
        drawTranslate = getInt2DVector(getProperty(collisionInfoStrings, "draw_translate").split(", "));
        ...
        fatherPosition = get3DVector(getProperty(collisionInfoStrings, "father_position").split(", "));
        ...
        for (int i = 0; i < modifiedCoords.length; i++) {
            modifiedCoords[i] = new Exo3DVector(
                Float.parseFloat(vertexStrings[i * 3]), Float.parseFloat(vertexStrings[(i * 3) + 1]), Float.parseFloat(vertexStrings[(i * 3) + 2])
            ).getTransformed(selfRotation, selfScale, selfPosition, fatherRotation, fatherScale, fatherPosition);
        }

        for (int i = 0; i < triangles.length; i++) {
            triangles[i] = new ExoTriangleVertices(Integer.parseInt(polygonStrings[i * 3]), Integer.parseInt(polygonStrings[(i * 3) + 1]), Integer.parseInt(polygonStrings[(i * 3) + 2]));
        }

        for (int i = 0; i < teamPlayerSpawns.length; i++) {
            teamPlayerSpawns[i] = new Exo2DVector(Float.parseFloat(teamPlayerSpawnStrings[i * 2]), Float.parseFloat(teamPlayerSpawnStrings[(i * 2) + 1]));
        }
        for (int i = 0; i < ffaPlayerSpawns.length; i++) {
            ffaPlayerSpawns[i] = new Exo2DVector(Float.parseFloat(ffaPlayerSpawnStrings[i * 2]), Float.parseFloat(ffaPlayerSpawnStrings[(i * 2) + 1]));
        }

        for (int i = 0; i < teamItemSpawns.length; i++) {
            teamItemSpawns[i] = new ExoItemSpawner(
                Integer.parseInt(teamItemSpawnStrings[i * 4]), Float.parseFloat(teamItemSpawnStrings[(i * 4) + 1]),
                Float.parseFloat(teamItemSpawnStrings[(i * 4) + 2]), Float.parseFloat(teamItemSpawnStrings[(i * 4) + 3])
            );
        }
        for (int i = 0; i < ffaItemSpawns.length; i++) {
            ffaItemSpawns[i] = new ExoItemSpawner(
                Integer.parseInt(ffaItemSpawnStrings[i * 4]), Float.parseFloat(ffaItemSpawnStrings[(i * 4) + 1]),
                Float.parseFloat(ffaItemSpawnStrings[(i * 4) + 2]), Float.parseFloat(ffaItemSpawnStrings[(i * 4) + 3])
            );
        }
    }

    private static ExoInt2DVector getInt2DVector (String[] pairStrings) {
        return new ExoInt2DVector(Integer.parseInt(pairStrings[0]), Integer.parseInt(pairStrings[1]));
    }

    private static Exo3DVector get3DVector (String[] tripletStrings) {
        return new Exo3DVector(Float.parseFloat(tripletStrings[0]), Float.parseFloat(tripletStrings[1]), Float.parseFloat(tripletStrings[2]));
    }
    ...
    public Image getImage (float scalar, ExoInt2DVector scaledDrawTranslate, ExoInt2DVector scaledDrawSize) {
        BufferedImage drawImage = new BufferedImage(scaledDrawSize.x, scaledDrawSize.y, BufferedImage.TYPE_INT_RGB);
        ...
        for (ExoTriangleVertices triangle : triangles) {
            ExoInt2DVector pointOne = modifiedCoords[triangle.vertexOne].convertNativeToDraw(scalar);
            ExoInt2DVector pointTwo = modifiedCoords[triangle.vertexTwo].convertNativeToDraw(scalar);
            ExoInt2DVector pointThree = modifiedCoords[triangle.vertexThree].convertNativeToDraw(scalar);
